...
class FPCoreParser(Parser):
    tokens = fpcore_lexer.FPCoreLexer.tokens
    ...
    def error(self, p):
        if p:
            print("Line {}: Syntax error at {}".format(p.lineno, p),
                  file=sys.stderr)
        else:
            print("Unexpected end of FPCore", file=sys.stderr)
        sys.exit(1)


parser = FPCoreParser()
def parse(text):
    lexed = fpcore_lexer.lex(text)
